...
# Ensure 'acres' column exists and filter out rows
...
# Calculate average property value per acre per bin and count per bin
avg_values = df_filtered.groupby("distance_bin", observed=True).agg({
    "property_value_per_acre": "mean", 
    "distance": "count"
...
'''
# Bar Graph (Distance vs. Average Property Value per Acre)
plt.figure(figsize=(10, 6))
sns.barplot(x="distance_mid", y="property_value_per_acre", data=avg_values, palette=colors_list)
...
'''
https://matplotlib.org/stable/api/_as_gen/matplotlib.figure.Figure.gca.html
'''
# Display the y-axis in full USD values
