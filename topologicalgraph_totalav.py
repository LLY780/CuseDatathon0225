from imports import pd, np, plt  # using modules imported via imports.py
...
# (Optional) Calculate distance for each parcel
...
# Define fixed bin boundaries (in USD)
boundaries = [0, 250000, 500000, 750000, 1e9]  # 1e9 as an upper cap for all values above $750K

# Define corresponding colors:
colors = [
    "#A020F0",  # $0 - $250K -> Purple
    "#0000FF",  # $250K - $500K -> Blue
    "#00FF00",  # $500K - $750K -> Green
    "#FFA500"   # $750K+ -> Orange
]
...
scatter = plt.scatter(parcelFile['longitude'], parcelFile['latitude'], 
                      c=parcelFile['property_value'], cmap=cmap, norm=norm,
                      alpha=0.7, edgecolor='k')

# Configure colorbar: set tick locations at the midpoint of each bin.
...
plt.title('Parcel Map: Total Value by Location')
